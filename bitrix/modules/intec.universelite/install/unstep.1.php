...
<form action="<?= $APPLICATION->GetCurPage() ?>" method="POST">
    ...
    <?php if (!empty($included)) { ?>
        <h3><?= Loc::getMessage('intec.universelite.installer.uninstall.modules.title') ?></h3>
        <div><?= Loc::getMessage('intec.universelite.installer.uninstall.modules.description') ?></div>
        <?php $number = 0 ?>
    ...
    <?php } ?>
    <div>
        <div style="margin-bottom: 10px;"><?= Loc::getMessage('intec.universelite.installer.uninstall.warning') ?></div>
        <input type="submit" value="<?= Loc::getMessage('intec.universelite.installer.uninstall.go')?>">
    </div>
<form>