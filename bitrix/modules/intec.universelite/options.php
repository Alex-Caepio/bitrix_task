 ...
 * @comment c0bebac64d66f7479e62b7b50b766416
 */

Loc::loadMessages(__FILE__);
$sModule = 'intec.universelite';
...
if ($sRights >= 'R') {
    ...
    foreach ($arSites as $arSite) {
        ...
        $arTabs[] = [
            'DIV' => $arSite['ID'],
            'TAB' => Loc::getMessage('intec.universelite.options.tab', $arSiteMacros),
            'ICON' => 'settings',
            'TITLE' => Loc::getMessage('intec.universelite.options.tab', $arSiteMacros)
        ];
...
?>
    <form method="POST">
        ...
        <?php $oTabControl->Buttons() ?>
            <input type="submit" class="adm-btn-save" name="Apply" value="<?= Loc::getMessage('intec.universelite.options.buttons.apply') ?>" />
            <input type="submit" name="Reset" value="<?= Loc::getMessage('intec.universelite.options.buttons.reset') ?>" />
        <?php $oTabControl->End() ?>
    </form>
<?php } ?>